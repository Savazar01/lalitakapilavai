...
export default function NavigationManagerPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 p-1.5 rounded-xl border border-slate-300 dark:border-slate-800 bg-slate-200/60 dark:bg-slate-900/90 shadow-2xs">
        {positions.map((pos) => {
          ...
          return (
            <Button
              ...
              className={`h-8 text-xs rounded-lg font-bold transition-all ${
                isActive
                  ? "bg-slate-900 text-white border border-slate-900 dark:bg-slate-100 dark:text-slate-900 dark:border-slate-100 shadow-xs"
                  : "bg-slate-100 text-slate-700 border border-slate-300 hover:bg-slate-200 hover:text-slate-900 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700"
              }`}
            >
              {pos.label}
            </Button>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {items.map((root, rootIdx) => (
            <Card key={root.id} className="overflow-hidden border border-slate-300 dark:border-slate-800 bg-white dark:bg-[#151B26] rounded-xl shadow-xs">
              {/* Root Item Header */}
              <div className="p-4 bg-white dark:bg-[#151B26] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 flex items-center justify-center font-mono font-bold text-xs text-slate-800 dark:text-slate-200">
                    {rootIdx + 1}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {root.openInNewTab && (
                        <Badge variant="outline" className="text-[10px] gap-1 border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300">
                ...
                <div className="flex items-center gap-1.5 self-end sm:self-center">
                  ...
                  <Button
                    ...
                    className="h-8 text-xs gap-1 border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800"
                  >
              ...
              {root.children && root.children.length > 0 && (
                <CardContent className="pt-0 pb-3 pl-8 sm:pl-12 pr-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 space-y-2">
                  <div className="pt-3 text-slate-700 dark:text-slate-300 font-bold text-xs tracking-wider uppercase">
                    Tier 2 Dropdown Links ({root.children.length})
                  </div>
                  {root.children.map((tier2, tier2Idx) => (
                    <div
                      key={tier2.id}
                      className="p-3.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800/90 flex flex-col sm:flex-row sm:items-center justify-between gap-2 shadow-2xs"
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <span className="text-[11px] font-mono font-semibold text-slate-600 dark:text-slate-400 ml-2">
                            {tier2.path}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1 self-end sm:self-center">
                        <Button
                          ...
                          className="h-7 w-7 p-0 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"
                        >
                        ...
                        <Button
                          ...
                          className="h-7 text-[11px] px-2 border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-semibold hover:bg-slate-100 dark:hover:bg-slate-800"
                        >
                      ...
                      {tier2.children && tier2.children.length > 0 && (
                        <div className="w-full pl-6 pt-2 space-y-1 border-t border-slate-200 dark:border-slate-700 mt-1">
                          <span className="text-[9px] uppercase font-mono font-bold text-slate-700 dark:text-slate-300 tracking-wider">
                            Tier 3 Nested Items:
                          </span>
                          {tier2.children.map((tier3) => (
                            <div
                              key={tier3.id}
                              className="flex items-center justify-between text-xs py-1.5 px-2.5 rounded border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 font-medium"
                            >
