...
export async function GET(request: NextRequest) {
  try {
    ...
    // 1. Fetch from Artwork model
    if (sourceFilter === "all" || sourceFilter === "artwork") {
    ...
    // 4. Scan public/media/public directory for uploaded files
    if (sourceFilter === "all" || sourceFilter === "storage") {
      try {
            ...
            if (!seenUrls.has(url)) {
              if (!search || f.toLowerCase().includes(search)) {
                ...
                items.push({
                  ...
                  title: f.replace(/[-_]/g, " "),
                  source: "storage",
                  category: "Uploaded Media Storage",
                });
              }
            }
          }
