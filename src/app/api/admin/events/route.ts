    ...
    const newEvent = await prisma.$transaction(async (tx) => {
      const ev = await tx.event.create({
        data: {
          ...
          galleryDisplayMode: galleryDisplayMode || "CAROUSEL",
          galleryAutoplayTimer: galleryAutoplayTimer !== undefined ? parseInt(galleryAutoplayTimer, 10) : 4,
          brochurePdfUrl: brochurePdfUrl || null,
          brochureTitle: brochureTitle || "Exhibition Monograph & Program Brochure",
          brochureDownloadable: brochureDownloadable !== undefined ? !!brochureDownloadable : true,
